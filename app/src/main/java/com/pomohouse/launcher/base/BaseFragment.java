...
/*    public WearerInfo getWatchInfoModel() {
    ...
    protected Bundle buildBundle(WearerInfo watchInfo) {
        bundle = new Bundle();
        bundle.putParcelable(WATCH_INFO, watchInfo);
        return bundle;
    }
    ...
    protected void addFragmentToBackStack(Fragment fragment, String TAG) {
        hideKeyboard();
        if (fragment != null && !getActivity().isFinishing()) {
            getActivity().getSupportFragmentManager().beginTransaction()
    ...
    protected void popFragmentFromNackStack(Fragment fragment) {
